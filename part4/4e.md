...
We want to set the initial status of newly created travel instances to _Open (O)_. To achieve this, we'll need to add a determination on modify with trigger operation `create` to the behavior definition. **Don't forget** to `use` the determination also in the behavior projection!  
