...
2. Enable the so-called optimistic lock for the Booking entity.
...
3. Generate UUID: we want to have automatically generated UUIDs (Booking UUID field) every time new instances are created.
...
4. Make the UUID fields of all parent and grand parent entities (like e.g. `TravelUUID` or `BookingUUID`) readonly: `field ( readonly ) TravelUUID`.

5. Define a mapping between the persistency table fields and the CDS view fields for the Booking entity.  
