...
Open the created class and click `Local Type` on the bottom - here we should have the local class. This is where we implement the logic for all validations, actions, determinations and feature control. Make sure the framework is also generating the `get_feature` method, otherwise we would have errors.  
