...
**Important:** The SAP Business Application Studio is not running as expected in some browsers, e.g. in Mozilla's _Firefox_ and especially with Apple's _Safari_. Therefore, **please use Google Chrome** (which is recommended by SAP) for all future modifications with the BAS!

As our Service Bindings/OData service is working fine, let’s use it within a SAP Fiori app. As the app doesn’t even exist yet, let’s create (or more precisely generate) it. Therefore, access the SAP Business Application Studio via your browser and enter your Fiori Dev Space. From the `Get Started` screen select the option *New project from template*:

![alt](images/image6_1.png)

[^ Top of page](#)  
Now choose the *SAP Fiori generator* in the *Select Template and Target Location* step of the Application Wizard provided by SAP Fiori Tools and press *Start*.

![alt](images/image6_2.png)

[^ Top of page](#)  
As part of the *Template Selection* step, we will pick the *List Report Page* floorplan and press *Next*.

![alt](images/image6_3.png)

[^ Top of page](#)  
In the next step, the *Data source* (Connect to a System), *System* (abap-evironment-unp (BTP)), and *Service* (your personal OData service ZRAPH_##_UI_TRAVELWDTP_V2) will be selected. Provide this information and press *Next >*.

![alt](images/image6_4.png)

[^ Top of page](#)  
In the *Entity Selection* step, we have to provide as *Main entity* the *Travel* entity and in *Navigation entity* please provide *None* since related navigation from the main entity to the child entities will be provided from backend via UI annotations. The *Table type* should be *Responsive*. After filling the fields, you can continue by pressing *Next >*.
...
[^ Top of page](#)  
Now press _Finish_ and wait for the magic to happen. After a few seconds your first SAP Fiori List Report app was generated and will automatically be shown in the SAP Business Application Studio. By expanding the project folder you can check out the generated files in the explorer.  

>**Remark** You might get some popup after the app was generated. Just press the blue button to `Open` the newly created project in your workspace.
...
[^ Top of page](#)  
In the appearing pop-up select the very first npm script *start*. You'll have to disable the popup blocker (if necessary). Now it'll take a few moments until it starts, but finally your app will appear in an additional browser tab. 
