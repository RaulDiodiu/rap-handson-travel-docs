...
* **Implementing Basic Authorizations**  
    ...
    * [Creating a CDS Access Control for ZRAPH_##_I_TravelWDTP](#markdown-header-creating-a-cds-access-control-for-zraph__i_travelwdtp)  
    * [Inheriting the CDS Access Control in ZRAPH_##_C_TravelWDTP](#markdown-header-inheriting-the-cds-access-control-in-zraph__c_travelwdtp)  
...
As work-around we'll now again adjust the condition in the underlying Access Control to `where true;`. This will grant us access to all entries again. Now you've learned, how you would normally handle roles and rights for specific users on which data they are allowed to see and which they aren't.