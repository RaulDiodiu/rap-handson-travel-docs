...
## Requirement
We have to provide a value help for the field `Hotel`, the problem arrives when our data is not provided via a CDS or at least stored in a database table.
...
To create the custom entity do the followign steps:
...
>Hint!   
If you didn't add the interface `IF_RAP_QUERY_PROVIDER` in the previous step to the query implementation class, you can add it now. Then use the quick fix `CTR+1` to add the implementation for the method `if_rap_query_provider`~`select`.
...
**The logic is as follows**: we have to write our code in the select method:  
    ...
    - call the function module `Z_RD_TRAVEL_API` to get the data.
...
- Add the annotation `@Consumption.valueHelpDefinition` to the field `HotelID` and removed other annotations from the field.   
- Delete or comment fields `_Hotel.Name`, `_Hotel.City` and association `_Hotel`.
- `Save` and `Activate`.

**Solution**  
```abap
      ...
      @Consumption.valueHelpDefinition: [ {entity: {name: 'zraph_ce_hotelextid_###', element: 'Hotel_ID' } } ]
      HotelID,
      ...
```
...
- Select the Travel consumption view from the list and press `Preview`.
- Press `Go` button and select one entry from the list.
- Go to `Room Reservation` table click `create`.
- Select a value for `HotelID` a new pop up should open like in the pic below.
