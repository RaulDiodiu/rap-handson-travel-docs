...
- Open the database table `ZRAPH_##_RoomRsv` and add a new field `Hotel_external_id`. On the UI we should see the label 'Hotel External ID' 
...
- Open the Behavior Definition `ZRAPH_##_I_TravelWDTP` and add the `HotelExternalID` to the mapping of entity `BookingSupplement`.
- `SAVE` and `ACTIVATE`
