...
In the CDS Views all fields, which are available in the database tables should be selected. Their key fields should be marked as key. For the amount fields the annotation `@Semantics.amount.currencyCode: '<currency field name>'` should be used to enrich the CDS view with semantical information. For the administrative fields you'll have to use the annotations `@Semantics.systemDateTime.localInstanceLastChangedAt: true` for all instances and additionally `@Semantics.user.createdBy: true`, `@Semantics.systemDateTime.createdAt: true`, `@Semantics.user.lastChangedBy: true` and `@Semantics.systemDateTime.lastChangedAt: true` for the travel root. 
