...
In the CDS Views all fields, which are available in the database tables should be selected. Their key fields should be marked as key. For the amount and currency fields the annotations `@Semantics.currencyCode: true` and `@Semantics.amount.currencyCode: '<currency field name>'` should be used to enrich the CDS view with semantical information.  

We’ll also have to expose the associations from Travel to Booking respective RoomReservation and the other way around (i.e. the composition of our BO tree). Same goes for the relationship between Booking and BookingSupplement. Don’t forget to also publish the associations from the composition tree.  
