...
### Introducting Annotation Syntax 

#### Header & Facet Configuration

To specify the header texts for Fiori UIs, the annotation `@UI.headerInfo` is used at entity level. It specifies the list title as well as the title for the object page. This annotation is actually define even before the `annotate entity` statement. You can check the syntax below:  
...
Enough introductory words - let us begin with creating the metadata extensions! We'll create them one after another starting with ZRAPH_##_C_TravelWDTP.
...
Next, make sure that the metadata layer is `#CORE`.

**TODO**  As we’ll add the actions acceptTravel **and** rejectTravel to the status element in `ZRAPH_##_C_TravelWDTP` later, we also have to expose these actions for the UI at field `TravelID` using the following syntax for **both** `lineItem` and `identification` annotations:

You can explicitly hide `LastChangedAt` with the annotation `@UI.hidden: true`. Also, please add the annotation `@UI.identification.criticality` for `FlightPrice` in the booking view referring to our field `Criticality`. For more information on all UI annotations, see [UI Annotations](https://help.sap.com/viewer/fc4c71aa50014fd1b43721701471913d/201909.001/en-US/5587d47763184cc48f164648b53c1e4f.html). 
...
## Overriding MDE Annotations via #LAYER
