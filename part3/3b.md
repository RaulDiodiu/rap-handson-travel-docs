...
* **Current: 3b. Adding master data associations to the VDM**  
    * [Requirement](#markdown-header-requirement)  
    * [Checking result via Data Preview](#markdown-header-checking-result-via-data-preview)  
    * [Adding Criticality field directly within the CDS View](#markdown-header-adding-criticality-field-directly-within-the-cds-view)   
...
Please add master data associations to the following CDS Views whereever possible: `/DMO/I_Agency`, `/DMO/I_Customer`, `/DMO/I_Carrier`, `/DMO/I_Connection`, `/DMO/I_Supplement`, `/DMO/I_SupplementText`, `ZRAPH_I_Hotel`, `ZRAPH_I_HotelRoomType`, `/DMO/I_Overall_Status_VH` and `I_Currency`.  

To do this, use the keyword `association`. You'll first have to check the key fields of those CDS views (can be checked via `F2`) to know, how to join them with our own CDS View. Again, publish those associations in the field list as well! As we want to ensure correct data, the association's cardinality is `[1..1]` meaning we're always expecting exactly one master data entry for the value stored in our own data.  
...
The value 3 will be shown as green in the Fiori App while 2 is yellow, 1 is red and 0 is neutral (grey). You can choose some thresholds on your own, just make sure that your values make sense using the Data Preview.  
